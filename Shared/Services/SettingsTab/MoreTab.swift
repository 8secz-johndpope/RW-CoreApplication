...
var settingsParentPage: RWReusableTableViewController {
    ...
    parentVC.addSection(RWSection([
        RWButton(router: "Preferences", to: settingsPreferencesPage)
    ]))
    
    parentVC.addSection(RWSection([
        RWButton(router: "Data", to: settingsDataPage),
        RWButton(router: "Notifications", to: settingsNotificationsPage),
        RWButton(router: "Security", to: settingsSecurityPage)
    ]))
...
var settingsPreferencesPage: RWReusableTableViewController {
...
var settingsDataPage: RWReusableTableViewController {
    let parentVC = RWReusableTableViewController()
...
var settingsNotificationsPage: RWReusableTableViewController {
    let parentVC = RWReusableTableViewController()
...
var settingsSecurityPage: RWReusableTableViewController {
    let parentVC = RWReusableTableViewController()
    //        parentVC.addSection(RPSection([
    //            RPButton(switch: "Use ", completion: {
    //
    //            }
    //        ]))
    return parentVC
